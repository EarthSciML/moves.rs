...
- All four Phase-6 control strategies (AVFT, Rate-of-Progress, OnRoadRetrofit, LEV)
...
**The Phase 4 data-plane wiring is not yet complete.** `moves run` correctly
...
Emission numbers will arrive with the Phase 4 data-plane milestone.
...
coding conventions, CI gates, and how to pick up a task from the migration plan.
Each numbered task in `moves-rust-migration-plan.md` is a self-contained unit
of work; the next major milestone (Phase 4, data-plane wiring) is ready for
contributors.
