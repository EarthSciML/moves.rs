...
slices the manifest by the RunSpec's active selections before
...
**Chained calculator** — no direct master-loop
...
- **`subscriptions()`** maps `(process, granularity, priority)` from the
 ...
 chained calculators return an empty slice.
...
- **`input_tables()`** lists canonical default-DB table names this calculator
 reads via `ctx.tables()`. The `InputDataManager` uses this list for
 lazy loading.
...
5. **Declare it in `input_tables()`.**
 ...
 `Calculator::input_tables()` so `InputDataManager` loads it.
...
joins the unified Parquet schema.
...
| `driver` | `nonroad.f`, `dayloop.f`, `daymthf.f`, `dispit.f` | Top-level SCC-group x geography x year loop |
...
| `emissions` | `clc*.f` and related | Emission-factor application per SCC x pollutant x model year |
...
2. **Numerical fidelity.** NONROAD has 30+ years of accumulated numerical
 ...
 warning in `docs/known-divergences.md`).
...
- **No Fortran FFI.** The FFI escape hatch available on native Linux (to
 ...
 Rust.
- **Single-threaded by default.** `run_simulation` must be callable from a
...
See `docs/performance-baseline.md` for the performance numbers and methodology,
