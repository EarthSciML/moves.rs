...
[`moves-rust-migration-plan.md`](../moves-rust-migration-plan.md). It covers
...
Each migration bead names the upstream source files it is porting — for
...
> (`mo-l006`). If you must search for a file, scope it to a bounded root: the
...
The bead description's source-path line numbers are relative to that commit.
...
With `$MOVES_SRC` set, the bead's source paths resolve directly — no search:
...
Each bead description follows the migration plan section for its task.
The typical artefacts to read are:
...
The migration plan section for your task lists which files to port and the
expected line counts. Use those counts as a sanity check that you're reading
the right file.
...
MOVES on the fixture suite (within the tolerance budgets in Tasks 73/74/115).
