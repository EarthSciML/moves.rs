...
It covers methodology, current (pre-data-plane) numbers, and the expected
comparison to canonical MOVES once the data plane is connected.
...
The target is a **5–20× wall-time improvement** on identical
...
## Pre-data-plane baseline (2026-05-21)
...
At the pre-data-plane framework-overhead level the Rust port completes **one
...
The target of **5–20× overall improvement** is conservative
...
- Concurrency tuning: sweep `--max-parallel-chunks` from 1
 to NCPU and document the throughput/RSS tradeoff.
- Concurrency correctness: verify byte-identical output at
 all parallelism levels.
- Public benchmark suite: full comparison against canonical
