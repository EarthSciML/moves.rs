...
It is the recommended hand-authored format for
...
3. **Open-set IDs keep both forms.** Pollutant, process, fuel-type,
 ...
 name is preserved for round-trip stability. Canonical enums for `Pollutant` /
 `EmissionProcess` / `SourceType` / `RoadType` will eventually make the `_name` companions
 derivable.
4. **Comments allowed.** TOML's `#` comments work anywhere — useful for
 annotating fixture selections, etc.
...
* **The XML serializer is byte-stable but not source-identical.** The
 serializer emits the canonical Java-style
...
* **Empty containers serialize as omitted in TOML, present in XML.**
 ...
 because that's what canonical MOVES does and what byte-stable re-serialization requires.
* **Pollutant/process names are user input today.** They round-trip
 through both formats but aren't validated against the MOVES
 defaultdb until canonical enum support is added. Mistyping `pollutant = "CO"` on a
