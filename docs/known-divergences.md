...
- Prints — **without asserting** — the divergence for fixtures with a known,
  reported data-plane bug (§4.4 below). Masking a real-bug divergence with a
  widened tolerance is worse than no gate, so those fixtures are quarantined,
  not tolerated.
...
**26 are quarantined as reported data-plane bugs**. The gate is green: it pins
the 8 working fixtures against regression while the 26 bugs remain on the record
(printed every run, never masked by a tolerance).
