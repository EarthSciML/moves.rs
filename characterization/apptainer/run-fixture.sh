...
#
# The rule, the patterns, and the measurements behind them live in
# lib/moves-log-scan.sh so that generate-corpus.sh (issue #60) scans by the
# same rule rather than growing a second, drifting copy. Read that file before
# changing anything here.
#
# shellcheck source=lib/moves-log-scan.sh
source "${HERE}/lib/moves-log-scan.sh"

scan_moves_log() {
    local reason
    if reason="$(moves_log_failure_reason "$1")"; then
        return 0
    fi
    fail "${reason}"
}
