...
//! default-DB Parquet layout.
//!
//! Pairs with the conversion pipeline in
...
//!
//! The `InputDataManager` is the eventual consumer:
//! it walks the RunSpec, builds [`TableFilter`] values per active
//! selection, and calls [`DefaultDb::scan`] for each input table. Until
//! lands, this crate is the de-facto entry point that downstream
//! importer crates (`CDB`, `PDB`, `Nonroad`, `AVFT`, `LEV/NLEV`)
//! can call directly.
//!
//! See `moves-rust-.md`, and
