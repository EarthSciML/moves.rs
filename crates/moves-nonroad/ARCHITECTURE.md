...
This document maps EPA's NONROAD2008a Fortran source
...
fixes the cross-cutting policies that the crate's modules must conform to.
...
called out explicitly so the boundary between " fixes this" and
...
NonroadOutputs`. No subprocess, no scratch files, no
MariaDB ingestion step. Output joins the unified Parquet output schema.
...
(see, 115, 116) and the 65 COMMON blocks.
...
audit in `moves-rust-md` § / "What's actually
...
port the routines as separate functions for fidelity.
 then refactors them into a single parameterized routine,
removing ~3,000 lines of duplication. The refactor is gated on
characterization-fixture parity.

### 2.3 `population`

Population apportionment, growth-factor application, age-distribution
and model-year fraction computation. (age distribution and
...
| Fortran file | Lines | Role |
|---|---|---|
| `clcems.f` | 360 | Exhaust emissions |
...
| `clcevems.f` | 721 | Evaporative emissions |
| `evemfclc.f` | 370 | Evaporative EF lookup |
| `clcrtrft.f` | 309 | Retrofit emissions |
...
include files. designs the typed Rust structs that hold
this state, grouped one struct per include file (`common::*`).
 ports `nonrdprm.inc` (parameter declarations and chemical
...
└── src/
 ...
 ├── main.rs ← native binary stub
 ├── error.rs ← Error / Result types
 ├── allocation.rs ← cluster 5
 ├── common/
 │ ├── mod.rs ← NonroadContext + sub-struct stubs
 │ └── consts.rs ← original Fortran dimensions + chemistry consts
 ├── driver.rs ← cluster 1
...
* `ndarray` is the planned multi-dimensional choice (rows × cols
 ...
 Rayon for parallelism (relevant for future).
...
* Numerical-fidelity work needs to detect
...
* ships `moves-nonroad` to the browser. WASM has
 ...
 *only* via the orchestrating `driver` layer (which has its own
 WASM gate via the orchestrator), and `std::io::*`.
...
* Floating-point math comes from `core::primitive::f64` methods,
 ...
 divergence surfaces in characterization runs and is
 triaged in. The skeleton does *not* attempt to mask
 this divergence.
* Threading is single-threaded by default. adds
...
target is added to CI in; until then, the gate is
...
| 134 | `rayon` | Parallelism (gated to non-WASM until wires up Web Workers) |
...
pub struct NonroadContext {
 ...
 // (other state buckets per's audit)
}
```

The split-by-include-file rule is the starting heuristic;
...
## 7. Integration with the moves-rs orchestrator

The orchestrator calls `moves-nonroad` directly — the
...
`getpop` SCC-group loop and the inner record loop, on top of the
 planner), dispatches every planned record to `geography`,
and returns a `NonroadOutputs` that the orchestrator merges into the
unified Parquet output. No subprocess, no scratch files, no
...
recording executor the numerical-fidelity harness
...
reference. The disk-side option-file orchestration and the
...
|---|---|---|---|
| 91 | NONROAD architecture map and Rust crate skeleton | (this) | (diff harness) |
...
establish foundations; 99–118 fill in the implementation
...
The following are explicitly out of scope for. They are
called out so that future tasks know the boundary.

* **Concrete `NonroadContext` fields.** Sketched in § 6 but
 finalized by. The skeleton's `NonroadContext` is empty.
...
* **Parser combinator vs. hand-written choice.** establishes
...
* **Parquet schema for NONROAD output.** defined
 the unified MOVES output schema; wires NONROAD into it.
 The Rust port emits the legacy NONROAD text format *and* Parquet;
 owns the schema-mapping decisions.
* **Parallelism.** Single-threaded by default; wires up
 Rayon-on-Web-Workers. The library does not depend on `rayon` at
 this stage.
* **Validation against the Windows reference.** owns the
 characterization-fixture diffing; the per-pollutant tolerance
 budget lives in `characterization/tolerance.toml` (see,
 work item ).
...
3. **`common` vs. `state`.** Both are reasonable; `common` is used
 ...
 `consts` carries the constants and `mod.rs` will
 carry the typed sub-structs.
4. **Workspace placement.** Co-located with `moves-snapshot` and
 `moves-fixture-capture` under `crates/`.'s orchestrator
...
* deliverables: `moves-snapshot` (canonical fixture format),
 `moves-fixture-capture` (intermediate dump → snapshot), 
