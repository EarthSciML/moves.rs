...
//! `clsnon`) are; assembling the populated callback
//! context the six geography routines need is the
...
//! of the integration; [`plan_scc_group`] is the inner loop
...
/// (`statcd`/`lstacd`, `fipcod`/`lfipcd`, `reglst`). The
...
/// Decide which geography routine(s) a record dispatches to/// `nonroad.f` :265–323.
///
...
/// Detect a growth-record pair and return its growth rate/// `nonroad.f` :206–219.
///
...
/// The result of planning one SCC group's record loop/// [`plan_scc_group`].
#[derive(Debug, Clone, PartialEq)]
