//! String utilities and small helpers.
//!
...
//! horsepower-category counter — but the groups them
//! here as miscellaneous small helpers. Both are
...
/// that nudges the maximum temperature by a 0.992 factor/// `adj_max = (max_tmp - min_tmp) * 0.992 + min_tmp` — to avoid
...
/// `fndhpc`. The Rust port takes `hpclev` as an explicit
/// slice parameter, which avoids dragging in either's
/// lookup module or `NonroadContext` for what is ~30
