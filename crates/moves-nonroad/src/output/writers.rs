//! Per-record output-file writers.
//!
...
//! output file are gone. The orchestrating layer (the
...
//! Windows reference build). Byte-exact line endings are a
//! fidelity checkpoint, not a concern.
//!
//! # The two output formats
//!
//! The calls for two output encodings: the
//! legacy NONROAD text format — ported here — and Apache Parquet on
//! the unified output schema (`moves-data`'s `output_schema`,
//!). The structured record types in this module
...
//! same records once the cross-crate wiring lands in the
...
/// SAROAD pollutant codes written into the AMS workfile/// `nonrdprm.inc` :366–370 (`ISCTHC`, `ISCNOX`, `ISCCO`, `ISCSOX`,
/// `ISCPM`).
