//! Message-file writers — `wrtmsg.f` and `wrtsum.f`.
//!
...
//! their lines out by column with the Fortran `T` (tab) descriptor//! see [`FortranLine`].
//!
...
//! state; the integration step populates it. See the
...
/// One state's population-record count and its counties/// `wrtsum.f`'s outer-loop row (`statcd(i)`, `statnm(i)`,
/// `nstarc(i)`).
#[derive(Debug, Clone, Copy)]
