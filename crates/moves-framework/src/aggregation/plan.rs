...
//! collapse to one. The deliverable here is the **column-shape plan**//! the list of group-by keys, aggregated-away columns, and `SUM(...)`
//! expressions — for each of the three output tables.
...
//! without a data plane: a test asserts the GROUP BY columns for
//! a given RunSpec configuration; wires the plan into the real
//! aggregator.
//!
//! See `moves-rust-.md`,.
//!
...
/// is left abstract here so the consumer can
...
///
/// Columns appear in the destination-schema order — consumes the
