...
//! the MasterLoop iterates over. This is the set of
//! `moves-rust-.md` calls "the sequence of
...
//! `County` rows.
//!
//! has no data plane, so callers build
//! [`GeographyTables`] from fixtures; will build it from the
...
/// by whoever builds the [`GeographyTables`] — fixtures in the
/// data plane later — so a `LinkRow` already carries the full
...
///
/// **input.** The Java producer queries a live database; the Rust
/// port takes the rows as data. callers (and the tests in this
/// module) build a `GeographyTables` from fixtures; data plane
