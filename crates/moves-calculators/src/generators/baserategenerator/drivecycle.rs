...
//! * `calculate_drive_cycle_op_mode_distribution`//! `calculateDriveCycleOpModeDistribution`: bin each second of a drive
...
///
/// Ports `processDriveCycles`. The `SaveROMD` debug table is not reproduced/// it has no effect on the generator's output.
#[must_use]
