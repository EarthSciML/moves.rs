    ...
    fn into_dataframe(rows: Vec<Self>) -> PolarsResult<DataFrame> {
        ...
        DataFrame::new(
            ...
            vec![
                ...
                Series::new(
                    "modelYearID".into(),
                    rows.iter().map(|r| r.model_year_id).collect::<Vec<i64>>(),
                )
                .into(),
                Series::new(
                    "sourceTypeID".into(),
                    rows.iter().map(|r| r.source_type_id).collect::<Vec<i64>>(),
                )
                .into(),
                Series::new(
                    "fuelTypeID".into(),
                    rows.iter().map(|r| r.fuel_type_id).collect::<Vec<i64>>(),
                )
                .into(),
                Series::new(
                    "engTechID".into(),
                    rows.iter().map(|r| r.eng_tech_id).collect::<Vec<i64>>(),
                )
                .into(),
                Series::new(
                    "regClassID".into(),
                    rows.iter().map(|r| r.reg_class_id).collect::<Vec<i64>>(),
                )
                .into(),
                Series::new(
                    "stmyFraction".into(),
    ...
    fn into_dataframe(rows: Vec<Self>) -> PolarsResult<DataFrame> {
        ...
        DataFrame::new(
            ...
            vec![
                Series::new(
                    "polProcessID".into(),
                    rows.iter().map(|r| r.pol_process_id).collect::<Vec<i64>>(),
                )
                .into(),
                Series::new(
                    "modelYearID".into(),
    ...
    fn into_dataframe(rows: Vec<Self>) -> PolarsResult<DataFrame> {
        ...
        DataFrame::new(
            ...
            vec![
                ...
                .into(),
                Series::new(
                    "shortModYrGroupID".into(),
                    rows.iter()
                        .map(|r| r.short_mod_yr_group_id)
                        .collect::<Vec<i64>>(),
                )
                .into(),
            ],
            ...
            .map(|i| {
                ...
                Ok(ModelYearGroupRow {
                    model_year_group_id: model_year_group_id
                        .get(i)
                        .ok_or_else(|| null("modelYearGroupID"))?,
                    short_mod_yr_group_id: short_mod_yr_group_id
                        .get(i)
                        .ok_or_else(|| null("shortModYrGroupID"))?,
                })
            })
            .collect()
    }
            ...
            .map(|i| {
                ...
                Ok(SourceTypePolProcessRow {
                    ...
                    is_reg_class_required: is_reg_class
                        .get(i)
                        .ok_or_else(|| null("isRegClassReqd"))?,
                    is_my_group_required: is_my_group
                        .get(i)
                        .ok_or_else(|| null("isMYGroupReqd"))?,
                })
            })
            .collect()
    }
    ...
    fn into_dataframe(rows: Vec<Self>) -> PolarsResult<DataFrame> {
        ...
        DataFrame::new(
            ...
            vec![
                ...
                Series::new(
                    "engSizeID".into(),
                    rows.iter().map(|r| r.eng_size_id).collect::<Vec<i64>>(),
                )
                .into(),
                Series::new(
                    "fuelTypeID".into(),
                ...
                Series::new(
                    "weightClassID".into(),
                    rows.iter().map(|r| r.weight_class_id).collect::<Vec<i64>>(),
                )
                .into(),
            ],
        )
    ...
    fn into_dataframe(rows: Vec<Self>) -> PolarsResult<DataFrame> {
        ...
        DataFrame::new(
            ...
            vec![
                Series::new(
                    "countyID".into(),
                    rows.iter().map(|r| r.county_id).collect::<Vec<i64>>(),
                )
                .into(),
                Series::new(
                    "fuelYearID".into(),
                    rows.iter().map(|r| r.fuel_year_id).collect::<Vec<i64>>(),
                )
                .into(),
                Series::new(
                    "modelYearGroupID".into(),
            ...
            .map(|i| {
                ...
                Ok(FuelUsageFractionRow {
                    ...
                    model_year_group_id: model_year_group_id
                        .get(i)
                        .ok_or_else(|| null("modelYearGroupID"))?,
                    source_bin_fuel_type_id: src_fuel_type_id
                        .get(i)
                        .ok_or_else(|| null("sourceBinFuelTypeID"))?,
                    fuel_supply_fuel_type_id: supply_fuel_type_id
                        .get(i)
    ...
    fn into_dataframe(rows: Vec<Self>) -> PolarsResult<DataFrame> {
        ...
        DataFrame::new(
            ...
            vec![
                ...
                Series::new(
                    "pollutantID".into(),
                    rows.iter().map(|r| r.pollutant_id).collect::<Vec<i64>>(),
                )
                .into(),
                Series::new(
                    "processID".into(),
                    rows.iter().map(|r| r.process_id).collect::<Vec<i64>>(),
                )
                .into(),
            ],
        )
    ...
    fn into_dataframe(rows: Vec<Self>) -> PolarsResult<DataFrame> {
        ...
        DataFrame::new(
            ...
            vec![
                Series::new(
                    "ageID".into(),
                    rows.iter().map(|r| r.age_id).collect::<Vec<i64>>(),
                )
                .into(),
                Series::new(
                    "ageGroupID".into(),
                    rows.iter().map(|r| r.age_group_id).collect::<Vec<i64>>(),
                )
                .into(),
            ],
        )
    ...
    fn into_dataframe(rows: Vec<Self>) -> PolarsResult<DataFrame> {
        ...
        DataFrame::new(
            ...
            vec![
                Series::new(
                    "yearID".into(),
                    rows.iter().map(|r| r.year_id).collect::<Vec<i64>>(),
                )
                .into(),
                Series::new(
                    "fuelYearID".into(),
...
impl Generator for SourceBinDistributionGenerator {
    ...
    fn execute(&self, ctx: &mut CalculatorContext) -> Result<CalculatorOutput, Error> {
        ...
        let myg: Vec<ModelYearGroupRow> = ctx.tables().iter_typed("ModelYearGroup")?;
        let stpp: Vec<SourceTypePolProcessRow> = ctx.tables().iter_typed("SourceTypePolProcess")?;
        let stmy: Vec<SourceTypeModelYearRow> = ctx.tables().iter_typed("SourceTypeModelYear")?;
        let rsft: Vec<RunSpecSourceFuelTypeRow> =
            ctx.tables().iter_typed("RunSpecSourceFuelType")?;
        let existing_bins: Vec<SourceBinRow> = ctx.tables().iter_typed("SourceBin")?;
        let existing_dist: Vec<SourceBinDistributionRow> =
            ctx.tables().iter_typed("SourceBinDistribution")?;
        let fuel_usage: Vec<FuelUsageFractionRow> = ctx.tables().iter_typed("fuelUsageFraction")?;
        ...
        let year_range = model_year_range(&run_spec_years, max_age_id).ok_or_else(|| {
            Error::Polars("Year table is empty; cannot derive the model-year range needed".into())
        })?;
        ...
        for &pol_process_id in &process_pol_process_ids {
            let out = pollutant_process_distribution(
                pol_process_id,
                &tables,
                &fuels_by_source_type,
                year_range,
            );
...
mod tests {
    ...
    fn execute_round_trip_produces_source_bin_and_distribution() {
        ...
        store.insert(
            "sampleVehiclePopulation",
            SampleVehiclePopulationRow::into_dataframe(vec![SampleVehiclePopulationRow {
                source_type_model_year_id: stmy_id,
        ...
        store.insert(
            "PollutantProcessModelYear",
            PollutantProcessModelYearRow::into_dataframe(vec![PollutantProcessModelYearRow {
                pol_process_id,
                model_year_id: year_id,
        ...
        store.insert(
            "ModelYearGroup",
            ModelYearGroupRow::into_dataframe(vec![ModelYearGroupRow {
                model_year_group_id: 5,
        ...
        store.insert(
            "SourceTypePolProcess",
            SourceTypePolProcessRow::into_dataframe(vec![SourceTypePolProcessRow {
                source_type_id: 21,
                pol_process_id,
        ...
        store.insert(
            "SourceTypeModelYear",
            SourceTypeModelYearRow::into_dataframe(vec![SourceTypeModelYearRow {
                source_type_model_year_id: stmy_id,
                source_type_id: 21,
            }])
            .unwrap(),
        ...
        store.insert(
            "RunSpecSourceFuelType",
            RunSpecSourceFuelTypeRow::into_dataframe(vec![RunSpecSourceFuelTypeRow {
                source_type_id: 21,
                fuel_type_id: 1,
            }])
            .unwrap(),
        ...
        store.insert(
            "fuelUsageFraction",
            FuelUsageFractionRow::into_dataframe(vec![FuelUsageFractionRow {
                county_id,
                fuel_year_id,
                model_year_group_id: 0,
                source_bin_fuel_type_id: 1,
