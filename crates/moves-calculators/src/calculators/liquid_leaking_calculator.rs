...
impl TableRow for PollutantProcessMappedModelYearRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let model_year = get_i32("modelYearID")?;
        let im_group = get_i32("IMModelYearGroupID")?;
        (0..df.height()).map(|i| {
            let null = |col: &'static str| row_err(t, i, col, "null value".into());
            Ok(PollutantProcessMappedModelYearRow {
                pol_process_id: pol_proc.get(i).ok_or_else(|| null("polProcessID"))?,
                model_year_id: model_year.get(i).ok_or_else(|| null("modelYearID"))?,
                im_model_year_group_id: im_group.get(i).ok_or_else(|| null("IMModelYearGroupID"))?,
            })
        }).collect()
    }
}
...
impl TableRow for ImFactorRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let test_standards = get_i32("testStandardsID")?;
        let src_type = get_i32("sourceTypeID")?;
        let fuel_type = get_i32("fuelTypeID")?;
        let im_group = get_i32("IMModelYearGroupID")?;
        let age_group = get_i32("ageGroupID")?;
        let im_factor = df.column("IMFactor").map_err(|e| row_err(t, 0, "IMFactor", e.to_string()))?.f64().map_err(|e| row_err(t, 0, "IMFactor", e.to_string()))?;
        (0..df.height()).map(|i| {
            let null = |col: &'static str| row_err(t, i, col, "null value".into());
            Ok(ImFactorRow {
                pol_process_id: pol_proc.get(i).ok_or_else(|| null("polProcessID"))?,
                inspect_freq: inspect_freq.get(i).ok_or_else(|| null("inspectFreq"))?,
                test_standards_id: test_standards.get(i).ok_or_else(|| null("testStandardsID"))?,
                source_type_id: src_type.get(i).ok_or_else(|| null("sourceTypeID"))?,
                fuel_type_id: fuel_type.get(i).ok_or_else(|| null("fuelTypeID"))?,
                im_model_year_group_id: im_group.get(i).ok_or_else(|| null("IMModelYearGroupID"))?,
                age_group_id: age_group.get(i).ok_or_else(|| null("ageGroupID"))?,
...
impl TableRow for ImCoverageRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let inspect_freq = get_i32("inspectFreq")?;
        let test_standards = get_i32("testStandardsID")?;
        let beg_my = get_i32("begModelYearID")?;
        let end_my = get_i32("endModelYearID")?;
        let compliance = df.column("complianceFactor").map_err(|e| row_err(t, 0, "complianceFactor", e.to_string()))?.f64().map_err(|e| row_err(t, 0, "complianceFactor", e.to_string()))?;
        (0..df.height()).map(|i| {
            let null = |col: &'static str| row_err(t, i, col, "null value".into());
            Ok(ImCoverageRow {
                pol_process_id: pol_proc.get(i).ok_or_else(|| null("polProcessID"))?,
                ...
                inspect_freq: inspect_freq.get(i).ok_or_else(|| null("inspectFreq"))?,
                test_standards_id: test_standards.get(i).ok_or_else(|| null("testStandardsID"))?,
                beg_model_year_id: beg_my.get(i).ok_or_else(|| null("begModelYearID"))?,
                end_model_year_id: end_my.get(i).ok_or_else(|| null("endModelYearID"))?,
                compliance_factor: compliance.get(i).ok_or_else(|| null("complianceFactor"))?,
            })
        }).collect()
    }
}
...
impl TableRow for EmissionRateByAgeRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let pol_proc = get_i32("polProcessID")?;
        let bin_id = df.column("sourceBinID").map_err(|e| row_err(t, 0, "sourceBinID", e.to_string()))?.i64().map_err(|e| row_err(t, 0, "sourceBinID", e.to_string()))?;
        let op_mode = get_i32("opModeID")?;
        let age_group = get_i32("ageGroupID")?;
        let base_rate = df.column("meanBaseRate").map_err(|e| row_err(t, 0, "meanBaseRate", e.to_string()))?.f64().map_err(|e| row_err(t, 0, "meanBaseRate", e.to_string()))?;
        let base_rate_im = df.column("meanBaseRateIM").map_err(|e| row_err(t, 0, "meanBaseRateIM", e.to_string()))?.f64().map_err(|e| row_err(t, 0, "meanBaseRateIM", e.to_string()))?;
        (0..df.height()).map(|i| {
            let null = |col: &'static str| row_err(t, i, col, "null value".into());
            Ok(EmissionRateByAgeRow {
                pol_process_id: pol_proc.get(i).ok_or_else(|| null("polProcessID"))?,
                source_bin_id: bin_id.get(i).ok_or_else(|| null("sourceBinID"))?,
                op_mode_id: op_mode.get(i).ok_or_else(|| null("opModeID"))?,
                age_group_id: age_group.get(i).ok_or_else(|| null("ageGroupID"))?,
                mean_base_rate: base_rate.get(i).ok_or_else(|| null("meanBaseRate"))?,
                mean_base_rate_im: base_rate_im.get(i).ok_or_else(|| null("meanBaseRateIM"))?,
            })
        }).collect()
    }
}
...
impl TableRow for FuelTypeRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        let t = "FuelType";
        let fuel_type = df.column("fuelTypeID").map_err(|e| row_err(t, 0, "fuelTypeID", e.to_string()))?.i32().map_err(|e| row_err(t, 0, "fuelTypeID", e.to_string()))?;
...
impl TableRow for SourceBinDistributionRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let pol_proc = get_i32("polProcessID")?;
        let bin_id = df.column("sourceBinID").map_err(|e| row_err(t, 0, "sourceBinID", e.to_string()))?.i64().map_err(|e| row_err(t, 0, "sourceBinID", e.to_string()))?;
        let fraction = df.column("sourceBinActivityFraction").map_err(|e| row_err(t, 0, "sourceBinActivityFraction", e.to_string()))?.f64().map_err(|e| row_err(t, 0, "sourceBinActivityFraction", e.to_string()))?;
        (0..df.height()).map(|i| {
            let null = |col: &'static str| row_err(t, i, col, "null value".into());
...
impl TableRow for SourceHoursRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let link = get_i32("linkID")?;
        let src_type = get_i32("sourceTypeID")?;
        let src_hours = df.column("sourceHours").map_err(|e| row_err(t, 0, "sourceHours", e.to_string()))?.f64().map_err(|e| row_err(t, 0, "sourceHours", e.to_string()))?;
        (0..df.height()).map(|i| {
            let null = |col: &'static str| row_err(t, i, col, "null value".into());
            Ok(SourceHoursRow {
                ...
                link_id: link.get(i).ok_or_else(|| null("linkID"))?,
                source_type_id: src_type.get(i).ok_or_else(|| null("sourceTypeID"))?,
                source_hours: src_hours.get(i).ok_or_else(|| null("sourceHours"))?,
            })
        }).collect()
    }
}
...
impl TableRow for OpModeDistributionRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let pol_proc = get_i32("polProcessID")?;
        let op_mode = get_i32("opModeID")?;
        let fraction = df.column("opModeFraction").map_err(|e| row_err(t, 0, "opModeFraction", e.to_string()))?.f64().map_err(|e| row_err(t, 0, "opModeFraction", e.to_string()))?;
        (0..df.height()).map(|i| {
            let null = |col: &'static str| row_err(t, i, col, "null value".into());
            Ok(OpModeDistributionRow {
                ...
                pol_process_id: pol_proc.get(i).ok_or_else(|| null("polProcessID"))?,
                op_mode_id: op_mode.get(i).ok_or_else(|| null("opModeID"))?,
                op_mode_fraction: fraction.get(i).ok_or_else(|| null("opModeFraction"))?,
            })
        }).collect()
    }
}
...
struct RunSpecMonthRow { month_id: i32 }
struct RunSpecHourDayRow { hour_day_id: i32 }
struct RunSpecSourceTypeRow { source_type_id: i32 }
...
impl TableRow for RunSpecMonthRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        let t = "RunSpecMonth"; let col = "monthID";
        let ids = df.column(col).map_err(|e| row_err(t, 0, col, e.to_string()))?.i32().map_err(|e| row_err(t, 0, col, e.to_string()))?;
        (0..df.height()).map(|i| Ok(RunSpecMonthRow { month_id: ids.get(i).ok_or_else(|| row_err(t, i, col, "null value".into()))? })).collect()
    }
}
...
impl TableRow for RunSpecHourDayRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        let t = "RunSpecHourDay"; let col = "hourDayID";
        let ids = df.column(col).map_err(|e| row_err(t, 0, col, e.to_string()))?.i32().map_err(|e| row_err(t, 0, col, e.to_string()))?;
        (0..df.height()).map(|i| Ok(RunSpecHourDayRow { hour_day_id: ids.get(i).ok_or_else(|| row_err(t, i, col, "null value".into()))? })).collect()
    }
}
...
impl TableRow for RunSpecSourceTypeRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        let t = "RunSpecSourceType"; let col = "sourceTypeID";
        let ids = df.column(col).map_err(|e| row_err(t, 0, col, e.to_string()))?.i32().map_err(|e| row_err(t, 0, col, e.to_string()))?;
        (0..df.height()).map(|i| Ok(RunSpecSourceTypeRow { source_type_id: ids.get(i).ok_or_else(|| row_err(t, i, col, "null value".into()))? })).collect()
    }
}

fn build_inputs(ctx: &CalculatorContext) -> Result<LiquidLeakingInputs, Error> {
    let tables = ctx.tables();
    let pos = ctx.position();
    let context = LiquidLeakingContext {
        year: pos.time.year.map(i32::from).unwrap_or(0),
        state_id: pos.location.state_id.map(|v| v as i32).unwrap_or(0),
        county_id: pos.location.county_id.map(|v| v as i32).unwrap_or(0),
        zone_id: pos.location.zone_id.map(|v| v as i32).unwrap_or(0),
        link_id: pos.location.link_id.map(|v| v as i32).unwrap_or(0),
    };
    Ok(LiquidLeakingInputs {
        context,
        pollutant_process_mapped_model_year: tables
            .iter_typed::<PollutantProcessMappedModelYearRow>("PollutantProcessMappedModelYear")?,
...
mod tests {
    ...
    fn execute_wires_through_data_plane() {
        ...
        store.insert(
            "PollutantProcessMappedModelYear",
            PollutantProcessMappedModelYearRow::into_dataframe(inputs.pollutant_process_mapped_model_year).unwrap(),
        );
        store.insert(
            "PollutantProcessAssoc",
        ...
        store.insert(
            "PollutantProcessModelYear",
            PollutantProcessModelYearRow::into_dataframe(inputs.pollutant_process_model_year).unwrap(),
        );
        store.insert(
            "RunSpecMonth",
            RunSpecMonthRow::into_dataframe(
                inputs.run_spec_month.iter().copied().map(|month_id| RunSpecMonthRow { month_id }).collect(),
            ).unwrap(),
        );
        store.insert(
            "RunSpecHourDay",
            RunSpecHourDayRow::into_dataframe(
                inputs.run_spec_hour_day.iter().copied().map(|hour_day_id| RunSpecHourDayRow { hour_day_id }).collect(),
            ).unwrap(),
        );
        store.insert(
            "RunSpecSourceType",
            RunSpecSourceTypeRow::into_dataframe(
                inputs.run_spec_source_type.iter().copied().map(|source_type_id| RunSpecSourceTypeRow { source_type_id }).collect(),
            ).unwrap(),
        );
