...
//!
//! # Relationship to (`HCSpeciationCalculator`)
//!
...
//! speciation formulas, the per-pollutant grouping into new fuel blocks//! and replaces the channel boundary with plain values: [`FuelBlock`]s in,
...
//! pulls NMHC in as an intermediate whenever NMOG or VOC is requested//! the result is numerically identical, and the degenerate set yields the
...
/// The Nonroad HC speciation lookup tables and the speciation algorithm/// the in-memory state and `calculate` body of the Go `nrhcspeciation`
/// package.
#[derive(Debug, Clone, Default)]
...
/// zero-sized type carries the calculator's chain metadata/// [`name`](Calculator::name), [`registrations`](Calculator::registrations),
