...
impl Calculator for NrHcSpeciationCalculator {
    ...
    /// body cannot read the `nrMethaneTHCRatio` / `nrHCSpeciation` tables
    /// nor the upstream THC fuel blocks, nor write the speciated blocks
    /// back. The faithful algorithm is ported and tested on
    /// [`NrHcSpeciation`]; once the `DataFrameStore` lands, `execute` builds
    /// an [`NrHcSpeciation`] from `ctx.tables()`, reads the THC
    /// [`FuelBlock`]s, applies
    /// [`speciate_block`](NrHcSpeciation::speciate_block), and stores the
