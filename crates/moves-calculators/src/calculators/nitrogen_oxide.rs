...
//!
//! [`Calculator::execute`] reads the five input tables from `ctx.tables()`
//! via [`DataFrameStoreTyped::iter_typed`], applying position filters that
//! mirror the SQL `##context.X##` macros: `PollutantProcessAssoc` is filtered
//! by `processID` and `MOVESWorkerOutput` by `(yearID, countyID, processID)`.
//! The result rows are converted to a `MOVESWorkerOutput` `DataFrame` via
//! [`crate::wiring::emit_rows`].
...
/// Minimal `SourceUseType` row — only `sourceTypeID` is needed (existence
/// filter: the SQL's `NOCopyOfSourceUseType` inner join).
...
// type `build_inputs` reads and for `MovesWorkerOutputRow` that `emit_rows`
// emits.
// ===========================================================================
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        (0..df.height()).map(|i| {
            Ok(SourceUseTypeIdRow {
                source_type_id: ids.get(i).ok_or_else(|| row_err(t, i, col, "null value".into()))?,
            })
        }).collect()
    }
}
...
/// Read all nitrogen-oxide input tables from `ctx.tables()`.
///
/// Applies position filters from [`crate::wiring::position_filter`]:
/// `PollutantProcessAssoc` is filtered by `processID` and `MOVESWorkerOutput`
/// by `(yearID, countyID, processID)`, mirroring the SQL `##context.X##`
/// macros.
fn build_inputs(ctx: &CalculatorContext) -> Result<NitrogenOxideInputs, Error> {
    ...
    Ok(NitrogenOxideInputs {
        ...
        pollutant_process_assoc: {
            let rows = tables.iter_typed::<PollutantProcessAssocRow>("PollutantProcessAssoc")?;
...
impl Calculator for NOCalculator {
    ...
    /// Read input tables from `ctx`, run the NO/HONO speciation algorithm, and
...
impl Calculator for NO2Calculator {
    ...
    /// Read input tables from `ctx`, run the NO2 speciation algorithm, and
    /// return the emission rows as a `MOVESWorkerOutput` `DataFrame`.
    fn execute(&self, ctx: &CalculatorContext) -> Result<CalculatorOutput, Error> {
        let inputs = build_inputs(ctx)?;
        let rows = self.calculate(&inputs);
...
mod tests {
    ...
    /// Build NO-style (polProcessID 3201, pollutantID 32) inputs — the
    /// NOCalculator produces NO rows; values chosen so one NOx row at ratio 0.7
    /// gives `emission_quant = 0.7 × 200.0 = 140.0`.
    fn no_minimal_inputs() -> NitrogenOxideInputs {
        NitrogenOxideInputs {
            no_no2_ratio: vec![NoNo2RatioRow {
                pol_process_id: 3201, // NO (32), running (1)
                source_type_id: 21,
    ...
    #[test]
    fn no_calculator_execute_wires_through_data_plane() {
        use moves_framework::DataFrameStore;
        let inputs = no_minimal_inputs();
        ...
        assert_eq!(df.height(), 1, "minimal inputs produce exactly one NO row");
        let quant = df.column("emissionQuant").unwrap().f64().unwrap().get(0).unwrap();
        assert!(quant > 0.0, "emissionQuant {quant} must be non-zero");
        // 0.7 × 200.0 = 140.0
        assert!((quant - 140.0).abs() < 1e-9, "emissionQuant {quant} != 140.0");
    }

    #[test]
    fn no2_calculator_execute_wires_through_data_plane() {
        use moves_framework::DataFrameStore;
        let inputs = minimal_inputs(); // NO2-style (polProcessID 3301, pollutantID 33)
        let mut store = moves_framework::InMemoryStore::new();
        store.insert("NONO2Ratio", NoNo2RatioRow::into_dataframe(inputs.no_no2_ratio).unwrap());
        store.insert("PollutantProcessAssoc", PollutantProcessAssocRow::into_dataframe(inputs.pollutant_process_assoc).unwrap());
        store.insert("PollutantProcessMappedModelYear", PollutantProcessModelYearRow::into_dataframe(inputs.pollutant_process_model_year).unwrap());
        ...
        let df = out.dataframe().expect("output should contain a DataFrame");
        assert_eq!(df.height(), 1, "minimal inputs produce exactly one NO2 row");
        let quant = df.column("emissionQuant").unwrap().f64().unwrap().get(0).unwrap();
        assert!(quant > 0.0, "emissionQuant {quant} must be non-zero");
        // 0.25 × 200.0 = 50.0
