...
//! running-exhaust activity that falls in a given `(regClass, fuelType)`//! the source-bin activity fractions summed over the engine-technology,
...
//! algorithm input; the SCC column is left to the output wiring.
//!
...
//! bug-for-bug is the/74 calculator-integration-validation call,
//! matching the / precedent. `SHO.distance` is likewise a
...
/// row. The output wiring writes it into the `activityTypeID`
/// column.
