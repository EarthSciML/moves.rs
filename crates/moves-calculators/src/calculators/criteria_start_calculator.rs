...
//! `database/CriteriaStartCalculator.sql` — migration plan Phase 3, Task 47.
//!
...
//! approach (migration-plan Task 45, `BaseRateCalculator`) superseded the
...
//!
//! The migration plan still lists the class as a task (Task 47), so this
...
//! * `POLY` (and any other value, the SQL `ELSE`) —
//!   `d × (tempAdjustTermA + d × (tempAdjustTermB + d × tempAdjustTermC))`.
//!
...
//! blocks), matching the Task 41 / Task 33 / Task 58 precedent. The `FLOAT`
...
// .sql`'s "Extract Data" section pulls. Following the Phase 3 convention
...
/// The per-run scalar context [`CriteriaStartCalculator::calculate`] reads —
...
/// One `MOVESWorkerOutput` row produced by the criteria start calculation —
/// the CSEC-8 output, with the I/M blend applied.
///
...
impl TableRow for FuelSupplyRow {
    ...
    fn into_dataframe(rows: Vec<Self>) -> PolarsResult<DataFrame> {
        let n = rows.len();
        DataFrame::new(n, vec![
            Series::new("fuelRegionID".into(), rows.iter().map(|r| r.fuel_region_id).collect::<Vec<i32>>()).into(),
...
fn met_start_adjustment(inputs: &CriteriaStartInputs) -> Vec<MetStartAdjustment> {
    ...
    let mut ppmy_by_key: HashMap<(i32, i32), Vec<&PollutantProcessMappedModelYearRow>> =
        HashMap::new();
    for ppmy in &inputs.pollutant_process_mapped_model_year {
        ppmy_by_key
...
fn emission_rates_with_im_and_temp(
    ...
    let mut msa_by_key: HashMap<(i32, i32, i32, i32), Vec<&MetStartAdjustment>> = HashMap::new();
...
fn met_source_bin_emission_rates(
    ...
    // join.
    let mut sbd_by_key: HashMap<(i32, i64), Vec<&SourceBinDistributionRow>> = HashMap::new();
...
fn activity_weighted_emission_rate_2(
    ...
    let zone_by_id: HashMap<i32, &ZoneRow> = inputs.zone.iter().map(|z| (z.zone_id, z)).collect();
...
fn assemble_emission_output(
    ...
    let mut awer2_by_key: HashMap<[i32; 7], Vec<&ActivityWeightedEmissionRate2>> = HashMap::new();
...
impl CriteriaStartCalculator {
    ...
    /// the master-loop context filter the Task 50 `execute` wiring uses; the
...
/// Default-DB / execution-DB tables the criteria start computation consumes —
...
impl Calculator for CriteriaStartCalculator {
    ...
    /// (`BaseRateCalculator`, Task 45) carries the criteria start-exhaust
    ...
    fn execute(&self, ctx: &CalculatorContext) -> Result<CalculatorOutput, Error> {
        ...
        let fuel_region_id = fuel_supply_rows.first().map(|r| r.fuel_region_id).unwrap_or(0);
...
mod tests {
    ...
    fn execute_returns_nonempty_dataframe_for_minimal_inputs() {
        ...
        store.insert("StartTempAdjustment", StartTempAdjustmentRow::into_dataframe(inputs.start_temp_adjustment.clone()).unwrap());
        store.insert("Starts", StartsRow::into_dataframe(inputs.starts.clone()).unwrap());
