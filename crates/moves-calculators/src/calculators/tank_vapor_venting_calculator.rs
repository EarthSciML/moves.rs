...
//! `database/TankVaporVentingCalculator.sql` —.//!.
//!
...
//!The pinned MOVES build compiles with the flag set, so the
...
//! directives regardless of which script runs). //! ports the single-day script; the Java `alterReplacementsAndSections`
...
//! on `EmissionRateByAge`, and `IMCoverage.countyID` / `IMCoverage.yearID`//! the port omits it as redundant given correctly-extracted inputs.
...
//! blocks), matching the / / precedent. Several
...
//! every row the calculator processes; following the precedent they
...
impl TableRow for CumTvvCoeffsRow {
    ...
    fn into_dataframe(rows: Vec<Self>) -> PolarsResult<DataFrame> {
        ...
        DataFrame::new(
            ...
            vec![
                ...
                Series::new(
                    "tvvTermA".into(),
                    rows.iter().map(|r| r.tvv_term_a).collect::<Vec<f64>>(),
                )
                .into(),
                Series::new(
                    "tvvTermB".into(),
                    rows.iter().map(|r| r.tvv_term_b).collect::<Vec<f64>>(),
                )
                .into(),
                Series::new(
                    "tvvTermC".into(),
                    rows.iter().map(|r| r.tvv_term_c).collect::<Vec<f64>>(),
                )
                .into(),
                Series::new(
                    "tvvTermAIM".into(),
                    rows.iter().map(|r| r.tvv_term_a_im).collect::<Vec<f64>>(),
                )
                .into(),
                Series::new(
                    "tvvTermBIM".into(),
                    rows.iter().map(|r| r.tvv_term_b_im).collect::<Vec<f64>>(),
                )
                .into(),
                Series::new(
                    "tvvTermCIM".into(),
                    rows.iter().map(|r| r.tvv_term_c_im).collect::<Vec<f64>>(),
                )
                .into(),
            ],
        )
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let ta = get_f64("tvvTermA")?;
        let tb = get_f64("tvvTermB")?;
        let tc = get_f64("tvvTermC")?;
        let taim = get_f64("tvvTermAIM")?;
        let tbim = get_f64("tvvTermBIM")?;
        let tcim = get_f64("tvvTermCIM")?;
        (0..df.height())
            .map(|i| {
                let null = |col: &'static str| row_err(t, i, col, "null value".into());
                Ok(CumTvvCoeffsRow {
...
/// the earliest hour. The SQL packs that ranking into one `max`/// `round(T,2)*100000 + (999 − hourID)`, unpacked with `mod(…,1000)`. The
