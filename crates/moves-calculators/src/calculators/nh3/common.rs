...
use moves_framework::{Error, IntoDataFrame, TableRow};
...
impl TableRow for ImFactorRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let pp = get_i32("polProcessID")?; let inf = get_i32("inspectFreq")?;
        let ts = get_i32("testStandardsID")?; let st = get_i32("sourceTypeID")?;
        let ft = get_i32("fuelTypeID")?; let im = get_i32("IMModelYearGroupID")?;
        ...
        (0..df.height()).map(|i| {
            ...
            Ok(ImFactorRow {
                ...
                im_model_year_group_id: im.get(i).ok_or_else(|| null("IMModelYearGroupID"))?,
...
impl TableRow for ImCoverageRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let pp = get_i32("polProcessID")?; let co = get_i32("countyID")?;
        let yr = get_i32("yearID")?; let st = get_i32("sourceTypeID")?;
        let ft = get_i32("fuelTypeID")?; let inf = get_i32("inspectFreq")?;
        let ts = get_i32("testStandardsID")?; let bm = get_i32("begModelYearID")?;
        let em = get_i32("endModelYearID")?;
        ...
        (0..df.height()).map(|i| {
            ...
            Ok(ImCoverageRow {
                ...
                inspect_freq: inf.get(i).ok_or_else(|| null("inspectFreq"))?,
                test_standards_id: ts.get(i).ok_or_else(|| null("testStandardsID"))?,
                beg_model_year_id: bm.get(i).ok_or_else(|| null("begModelYearID"))?,
                end_model_year_id: em.get(i).ok_or_else(|| null("endModelYearID"))?,
...
impl TableRow for PollutantProcessMappedModelYearRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let im = get_i32("IMModelYearGroupID")?;
        (0..df.height()).map(|i| {
            let null = |col: &'static str| row_err(t, i, col, "null value".into());
...
impl TableRow for AgeCategoryRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let ag = get_i32("ageID")?; let gr = get_i32("ageGroupID")?;
        (0..df.height()).map(|i| {
            let null = |col: &'static str| row_err(t, i, col, "null value".into());
            Ok(AgeCategoryRow {
                age_id: ag.get(i).ok_or_else(|| null("ageID"))?,
                age_group_id: gr.get(i).ok_or_else(|| null("ageGroupID"))?,
            })
        }).collect()
    }
}
...
impl TableRow for PollutantProcessAssocRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let pp = get_i32("polProcessID")?; let pr = get_i32("processID")?; let pl = get_i32("pollutantID")?;
        (0..df.height()).map(|i| {
            let null = |col: &'static str| row_err(t, i, col, "null value".into());
...
impl TableRow for EmissionRateByAgeRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let mi = df.column("meanBaseRateIM").map_err(|e| row_err(t, 0, "meanBaseRateIM", e.to_string()))?.f64().map_err(|e| row_err(t, 0, "meanBaseRateIM", e.to_string()))?;
        (0..df.height()).map(|i| {
            let null = |col: &'static str| row_err(t, i, col, "null value".into());
            Ok(EmissionRateByAgeRow {
                ...
                mean_base_rate_im: mi.get(i).ok_or_else(|| null("meanBaseRateIM"))?,
            })
        }).collect()
    }
}
...
impl TableRow for OpModeDistributionRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let st = get_i32("sourceTypeID")?; let hd = get_i32("hourDayID")?;
        let lk = get_i32("linkID")?; let pp = get_i32("polProcessID")?; let om = get_i32("opModeID")?;
