...
impl HcSpeciation {
    ...
    /// volToWtPercentOxy * totalOxygenate`; with no profile row found, the
    /// result is `None` (the row is dropped, matching canonical's INNER JOIN).
    ...
    fn speciate_nmog_or_voc(
        ...
        // taken there and their non-zero NMOG/VOC are unaffected. (The audit's
        // FINDING 1 reintroduced the Go zero-emit; it regressed process-refueling
        // — see [[process-refueling-false-graduation]] — so it is reverted here.)
...
impl Calculator for HcSpeciationCalculator {
    ...
    fn execute(&self, ctx: &CalculatorContext) -> Result<CalculatorOutput, Error> {
        ...
        for row in &thc_rows {
            ...
            let Some(supply) = hc_fuel_supply.get(&(row.year_id, row.month_id, row.fuel_type_id))
            else {
                continue;
            };
            let emissions: Vec<Emission> = supply
                .iter()
                .map(
                    |&(fuel_sub_type_id, fuel_formulation_id, market_share)| Emission {
                        fuel_sub_type_id,
                        fuel_formulation_id,
...
mod tests {
    ...
    fn execute_wires_through_data_plane() {
        ...
        store.insert(
            "MonthOfAnyYear",
            DataFrame::new(1, vec![col_i32("monthID", 7), col_i32("monthGroupID", 7)]).unwrap(),
        );
        store.insert(
            "FuelSubtype",
            DataFrame::new(
