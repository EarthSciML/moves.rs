...
//! `TankVaporVentingCalculator.sql` () — the **tank
...
//! the `USE_MULTIDAY_DIURNALS` compilation flag//! `MultidayTankVaporVentingCalculator.sql` (this port) when the flag is set,
//! `TankVaporVentingCalculator.sql` otherwise. The pinned MOVES
...
//! and are scoped out exactly as the port scoped out "Extract Data":
//!
...
//! calculator-integration-validation task (). The schema-nullable
...
//! tables are placeholders until the `DataFrameStore` lands
//! (), so `execute` cannot yet read the inputs nor
...
/// carry `leakFraction` / `leakFractionIM` columns, but no step reads them/// TVV-5 takes the leak fractions from `stmyTVVEquations` — so the port omits
...
/// the earliest hour. The SQL packs that ranking into one `max`/// `round(T,2)*100000 + (999 − hourID)`, unpacked with `mod(…,1000)`. The port
...
/// `priorHourID` is the cyclic predecessor of `hourID` within `1..=24`/// `mod(hourID − 2 + 24, 24) + 1` — pre-computed for TVV-6's self-join.
...
/// The RVP-interpolated cubic adjustment-polynomial terms for one fuel type/// `averageTankGasoline.adjustTerm3 / 2 / 1` and `adjustConstant`.
#[derive(Debug, Clone, Copy, PartialEq)]
struct AdjustTerms {
    term3: f64,
    term2: f64,
...
/// Compute the RVP-interpolated `averageTankGasoline.adjustTerm*` columns/// the SQL block that rebuilds `averageTankGasoline` before TVV-8.
///
