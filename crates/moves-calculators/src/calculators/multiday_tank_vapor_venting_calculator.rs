...
impl TableRow for CountyRow {
    ...
    fn into_dataframe(rows: Vec<Self>) -> PolarsResult<DataFrame> {
        ...
        DataFrame::new(n, vec![
            ...
            Series::new("barometricPressure".into(), rows.iter().map(|r| r.barometric_pressure).collect::<Vec<f64>>()).into(),
        ])
    }
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        let t = "County";
        ...
        let baro_col = df.column("barometricPressure").map_err(|e| row_err(t, 0, "barometricPressure", e.to_string()))?.f64().map_err(|e| row_err(t, 0, "barometricPressure", e.to_string()))?;
        (0..df.height()).map(|i| {
            let null = |col: &'static str| row_err(t, i, col, "null value".into());
            Ok(CountyRow {
                county_id: county_id_col.get(i).ok_or_else(|| null("countyID"))?,
                barometric_pressure: baro_col.get(i).ok_or_else(|| null("barometricPressure"))?,
            })
        }).collect()
    }
}
...
impl TableRow for EvapTemperatureAdjustmentRow {
    fn table_name() -> &'static str { "evapTemperatureAdjustment" }
    fn polars_schema() -> Schema {
        Schema::from_iter([
    ...
    fn into_dataframe(rows: Vec<Self>) -> PolarsResult<DataFrame> {
        let n = rows.len();
        DataFrame::new(n, vec![
            Series::new("tempAdjustTerm3".into(), rows.iter().map(|r| r.temp_adjust_term_3).collect::<Vec<f64>>()).into(),
            Series::new("tempAdjustTerm2".into(), rows.iter().map(|r| r.temp_adjust_term_2).collect::<Vec<f64>>()).into(),
            Series::new("tempAdjustTerm1".into(), rows.iter().map(|r| r.temp_adjust_term_1).collect::<Vec<f64>>()).into(),
            Series::new("tempAdjustConstant".into(), rows.iter().map(|r| r.temp_adjust_constant).collect::<Vec<f64>>()).into(),
        ])
    }
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let t3 = get_f64("tempAdjustTerm3")?; let t2 = get_f64("tempAdjustTerm2")?;
        let t1 = get_f64("tempAdjustTerm1")?; let tc = get_f64("tempAdjustConstant")?;
        (0..df.height()).map(|i| {
            let null = |col: &'static str| row_err(t, i, col, "null value".into());
...
impl TableRow for EvapRvpTemperatureAdjustmentRow {
    fn table_name() -> &'static str { "evapRVPTemperatureAdjustment" }
    fn polars_schema() -> Schema {
        Schema::from_iter([
            ("fuelTypeID".into(), DataType::Int32),
            ("RVP".into(), DataType::Float64),
    ...
    fn into_dataframe(rows: Vec<Self>) -> PolarsResult<DataFrame> {
        ...
        DataFrame::new(n, vec![
            Series::new("fuelTypeID".into(), rows.iter().map(|r| r.fuel_type_id).collect::<Vec<i32>>()).into(),
            Series::new("RVP".into(), rows.iter().map(|r| r.rvp).collect::<Vec<f64>>()).into(),
            Series::new("adjustTerm3".into(), rows.iter().map(|r| r.adjust_term_3).collect::<Vec<f64>>()).into(),
            Series::new("adjustTerm2".into(), rows.iter().map(|r| r.adjust_term_2).collect::<Vec<f64>>()).into(),
            Series::new("adjustTerm1".into(), rows.iter().map(|r| r.adjust_term_1).collect::<Vec<f64>>()).into(),
            Series::new("adjustConstant".into(), rows.iter().map(|r| r.adjust_constant).collect::<Vec<f64>>()).into(),
        ])
    }
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let rvp = get_f64("RVP")?;
        let a3 = get_f64("adjustTerm3")?; let a2 = get_f64("adjustTerm2")?;
        let a1 = get_f64("adjustTerm1")?; let ac = get_f64("adjustConstant")?;
        (0..df.height()).map(|i| {
            let null = |col: &'static str| row_err(t, i, col, "null value".into());
...
impl TableRow for SampleVehicleSoakingRow {
    fn table_name() -> &'static str { "sampleVehicleSoaking" }
    fn polars_schema() -> Schema {
        Schema::from_iter([
    ...
    fn into_dataframe(rows: Vec<Self>) -> PolarsResult<DataFrame> {
        let n = rows.len();
        DataFrame::new(n, vec![
            Series::new("soakDayID".into(), rows.iter().map(|r| r.soak_day_id).collect::<Vec<i32>>()).into(),
            Series::new("sourceTypeID".into(), rows.iter().map(|r| r.source_type_id).collect::<Vec<i32>>()).into(),
            Series::new("dayID".into(), rows.iter().map(|r| r.day_id).collect::<Vec<i32>>()).into(),
            Series::new("hourID".into(), rows.iter().map(|r| r.hour_id).collect::<Vec<i32>>()).into(),
            Series::new("soakFraction".into(), rows.iter().map(|r| r.soak_fraction).collect::<Vec<f64>>()).into(),
        ])
    }
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let sd = get_i32("soakDayID")?; let st = get_i32("sourceTypeID")?;
        let day = get_i32("dayID")?; let hr = get_i32("hourID")?;
        let sf = df.column("soakFraction").map_err(|e| row_err(t, 0, "soakFraction", e.to_string()))?.f64().map_err(|e| row_err(t, 0, "soakFraction", e.to_string()))?;
        (0..df.height()).map(|i| {
            let null = |col: &'static str| row_err(t, i, col, "null value".into());
...
impl TableRow for StmyTvvCoeffsRow {
    fn table_name() -> &'static str { "stmyTVVCoeffs" }
    fn polars_schema() -> Schema {
        Schema::from_iter([
            ("sourceTypeID".into(), DataType::Int32),
            ("modelYearID".into(), DataType::Int32),
    ...
    fn into_dataframe(rows: Vec<Self>) -> PolarsResult<DataFrame> {
        ...
        DataFrame::new(n, vec![
            ...
            Series::new("fuelTypeID".into(), rows.iter().map(|r| r.fuel_type_id).collect::<Vec<i32>>()).into(),
            Series::new("polProcessID".into(), rows.iter().map(|r| r.pol_process_id).collect::<Vec<i32>>()).into(),
            Series::new("backPurgeFactor".into(), rows.iter().map(|r| r.back_purge_factor).collect::<Vec<f64>>()).into(),
            Series::new("averageCanisterCapacity".into(), rows.iter().map(|r| r.average_canister_capacity).collect::<Vec<f64>>()).into(),
            Series::new("leakFraction".into(), rows.iter().map(|r| r.leak_fraction).collect::<Vec<f64>>()).into(),
            Series::new("leakFractionIM".into(), rows.iter().map(|r| r.leak_fraction_im).collect::<Vec<f64>>()).into(),
            Series::new("tankSize".into(), rows.iter().map(|r| r.tank_size).collect::<Vec<f64>>()).into(),
            Series::new("tankFillFraction".into(), rows.iter().map(|r| r.tank_fill_fraction).collect::<Vec<f64>>()).into(),
        ])
    }
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        let t = "stmyTVVCoeffs";
        ...
        let st = get_i32("sourceTypeID")?; let my = get_i32("modelYearID")?;
        let ft = get_i32("fuelTypeID")?; let pp = get_i32("polProcessID")?;
        let bp = get_f64("backPurgeFactor")?; let acc = get_f64("averageCanisterCapacity")?;
        let lf = get_f64("leakFraction")?; let lfim = get_f64("leakFractionIM")?;
        let ts = get_f64("tankSize")?; let tff = get_f64("tankFillFraction")?;
        (0..df.height()).map(|i| {
            let null = |col: &'static str| row_err(t, i, col, "null value".into());
            Ok(StmyTvvCoeffsRow {
                source_type_id: st.get(i).ok_or_else(|| null("sourceTypeID"))?,
                ...
                average_canister_capacity: acc.get(i).ok_or_else(|| null("averageCanisterCapacity"))?,
...
impl TableRow for StmyTvvEquationsRow {
    fn table_name() -> &'static str { "stmyTVVEquations" }
    fn polars_schema() -> Schema {
        Schema::from_iter([
            ("sourceTypeID".into(), DataType::Int32),
            ("modelYearID".into(), DataType::Int32),
            ...
            ("backPurgeFactor".into(), DataType::Float64),
            ("averageCanisterCapacity".into(), DataType::Float64),
            ("regClassFractionOfSourceTypeModelYearFuel".into(), DataType::Float64),
    ...
    fn into_dataframe(rows: Vec<Self>) -> PolarsResult<DataFrame> {
        ...
        DataFrame::new(n, vec![
            ...
            Series::new("backPurgeFactor".into(), rows.iter().map(|r| r.back_purge_factor).collect::<Vec<f64>>()).into(),
            Series::new("averageCanisterCapacity".into(), rows.iter().map(|r| r.average_canister_capacity).collect::<Vec<f64>>()).into(),
            Series::new("regClassFractionOfSourceTypeModelYearFuel".into(), rows.iter().map(|r| r.reg_class_fraction_of_source_type_model_year_fuel).collect::<Vec<f64>>()).into(),
            Series::new("tvvEquation".into(), rows.iter().map(|r| r.tvv_equation.clone()).collect::<Vec<String>>()).into(),
            Series::new("leakEquation".into(), rows.iter().map(|r| r.leak_equation.clone()).collect::<Vec<String>>()).into(),
            Series::new("leakFraction".into(), rows.iter().map(|r| r.leak_fraction).collect::<Vec<f64>>()).into(),
            Series::new("leakFractionIM".into(), rows.iter().map(|r| r.leak_fraction_im.unwrap_or(f64::NAN)).collect::<Vec<f64>>()).into(),
            Series::new("tankSize".into(), rows.iter().map(|r| r.tank_size).collect::<Vec<f64>>()).into(),
            Series::new("tankFillFraction".into(), rows.iter().map(|r| r.tank_fill_fraction).collect::<Vec<f64>>()).into(),
        ])
    }
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let st = get_i32("sourceTypeID")?; let my = get_i32("modelYearID")?;
        let ft = get_i32("fuelTypeID")?; let pp = get_i32("polProcessID")?; let rc = get_i32("regClassID")?;
        let bp = get_f64("backPurgeFactor")?; let acc = get_f64("averageCanisterCapacity")?;
        let rcf = get_f64("regClassFractionOfSourceTypeModelYearFuel")?;
        let tvveq_col = df.column("tvvEquation").map_err(|e| row_err(t, 0, "tvvEquation", e.to_string()))?.str().map_err(|e| row_err(t, 0, "tvvEquation", e.to_string()))?;
        let leakeq_col = df.column("leakEquation").map_err(|e| row_err(t, 0, "leakEquation", e.to_string()))?.str().map_err(|e| row_err(t, 0, "leakEquation", e.to_string()))?;
        let lf = get_f64("leakFraction")?; let lfim = get_f64("leakFractionIM")?;
        let ts = get_f64("tankSize")?; let tff = get_f64("tankFillFraction")?;
        (0..df.height()).map(|i| {
            let null = |col: &'static str| row_err(t, i, col, "null value".into());
            Ok(StmyTvvEquationsRow {
                ...
                back_purge_factor: bp.get(i).ok_or_else(|| null("backPurgeFactor"))?,
                average_canister_capacity: acc.get(i).ok_or_else(|| null("averageCanisterCapacity"))?,
                reg_class_fraction_of_source_type_model_year_fuel: rcf.get(i).ok_or_else(|| null("regClassFractionOfSourceTypeModelYearFuel"))?,
                tvv_equation: tvveq_col.get(i).ok_or_else(|| null("tvvEquation"))?.to_string(),
                leak_equation: leakeq_col.get(i).ok_or_else(|| null("leakEquation"))?.to_string(),
                leak_fraction: lf.get(i).ok_or_else(|| null("leakFraction"))?,
...
impl TableRow for MultidayTankVaporVentingEmissionRow {
    ...
    fn from_dataframe(df: &DataFrame) -> moves_framework::Result<Vec<Self>> {
        ...
        let proc = get_i32("processID")?; let st = get_i32("sourceTypeID")?; let rc = get_i32("regClassID")?;
        let ft = get_i32("fuelTypeID")?; let my = get_i32("modelYearID")?; let rt = get_i32("roadTypeID")?;
...
impl Calculator for MultidayTankVaporVentingCalculator {
    ...
    fn execute(&self, ctx: &CalculatorContext) -> Result<CalculatorOutput, Error> {
        ...
        let inputs = MultidayTankVaporVentingInputs {
            ...
            evap_temperature_adjustment: tables.iter_typed::<EvapTemperatureAdjustmentRow>("evapTemperatureAdjustment")?,
            evap_rvp_temperature_adjustment: tables.iter_typed::<EvapRvpTemperatureAdjustmentRow>("evapRVPTemperatureAdjustment")?,
            ...
            sample_vehicle_soaking: tables.iter_typed::<SampleVehicleSoakingRow>("sampleVehicleSoaking")?,
...
mod tests {
    ...
    fn execute_returns_nonempty_dataframe_for_minimal_inputs() {
        ...
        store.insert("evapTemperatureAdjustment", EvapTemperatureAdjustmentRow::into_dataframe(inputs.evap_temperature_adjustment.clone()).unwrap());
        store.insert("evapRVPTemperatureAdjustment", EvapRvpTemperatureAdjustmentRow::into_dataframe(inputs.evap_rvp_temperature_adjustment.clone()).unwrap());
        ...
        store.insert("sampleVehicleSoaking", SampleVehicleSoakingRow::into_dataframe(inputs.sample_vehicle_soaking.clone()).unwrap());
        ...
        store.insert("stmyTVVCoeffs", StmyTvvCoeffsRow::into_dataframe(inputs.stmy_tvv_coeffs.clone()).unwrap());
        store.insert("stmyTVVEquations", StmyTvvEquationsRow::into_dataframe(inputs.stmy_tvv_equations.clone()).unwrap());
