...
use moves_framework::{CalculatorContext, DataFrameStoreTyped, Error, TableRow};
...
/// Build the [`WtpInputs`] bundle from the [`CalculatorContext`] slow-tier
/// store — the bucket-A data-plane wiring shared by [`super::total_energy`],
/// [`super::co2_atmospheric`], and [`super::ch4n2o`].
pub(crate) fn build_wtp_inputs(ctx: &CalculatorContext) -> Result<WtpInputs, Error> {
    let tables = ctx.tables();
    let filter = crate::wiring::position_filter(ctx);
    let target_year = filter.year.unwrap_or(0);
    Ok(WtpInputs {
