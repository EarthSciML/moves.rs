//! Exhaust particulate-matter calculators —.
//!
//! Ports the two PM-exhaust calculators the groups into
//!:
//!
...
//! * [`running::BasicRunningPmEmissionCalculator`]//! `BasicRunningPMEmissionCalculator`, the direct subscriber that computes
...
//! module because ports them together. `BasicStartPMEmissionCalculator`
//! the start-exhaust counterpart — is a separate task (
//!) and is not ported here.
//!
//! Each calculator's module documents its source files, the SQL pipeline it
//! ports, its chain metadata, and the data-plane status.
